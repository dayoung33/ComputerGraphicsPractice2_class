						   ...
						   D3DXMATRIX baseViewMatrix)
{
	...
	result = UpdateSentence(m_sentence1, "Hello", 300, 10, 1.0f, 1.0f, 1.0f, deviceContext);
	if(!result)
	{
		return false;
	}
	...
	// Now update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence2, "Goodbye", 300, 30, 1.0f, 1.0f, 0.0f, deviceContext);
	if(!result)
	{
		return false;
	}
