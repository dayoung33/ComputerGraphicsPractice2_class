...
bool MainScene::Frame(int mouseX, int mouseY)
{
	bool result;
...
bool MainScene::Render(float rotation)
{
...
void MainScene::CamMoveLR(float x)
{
	m_Camera->MoveLeftRight(x);
}

void MainScene::CamMoveBF(float z)
{
	m_Camera->MoveBackForward(z);
}

void MainScene::CamLookRotate(DIMOUSESTATE & curMouse, DIMOUSESTATE & nextMouse)
{
	m_Camera->LookRotation(curMouse, nextMouse);
}

void MainScene::SetInput(InputClass * _Input)
{
	m_Input = _Input;
}
